...
module.exports.Vip = function(request, response){
  ...
  async.parallel ([
    ...
    function(callback){
      model.estChanteur(num,function (err, result){callback(null,result)});
    },
    function(callback){
      model.getInfoChanteur(num,function (err, result){callback(null,result)});
    },
    function(callback){
      model.estCouturier(num,function (err, result){callback(null,result)});
    },
    function(callback){
      model.estRealisateur(num,function (err, result){callback(null,result)});
    },
    function(callback){
      model.getInfoRealisateur(num,function (err, result){callback(null,result)});
    },
  ],
  async function (err,result){
    ...
    response.estChanteur = result[9];
    response.getInfoChanteur = result[10];
    response.estCouturier = result[11];
    response.estRealisateur = result[12];
    response.getInfoRealisateur = result[13];
